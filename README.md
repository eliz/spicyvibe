...
Step-by-Step for Attendees:
1. Terminal Check: Open your terminal (VSCode integrated terminal is best).  

2. Install: Run the one-command installer:
curl -fsSL [https://opencode.ai/install](https://opencode.ai/install) | bash

3. Connect AI: Run /connect in your terminal.

- Pro Tip: For the workshop, recommend OpenCode Zen. It’s a pre-configured, curated model list that doesn't require individual OpenAI/Anthropic keys—great for a group setting.  

4. Authorize: Follow the link to opencode.ai/auth to sign in and grab your session token.

5. Initialize Project: Navigate to your project folder and run opencode /init. This creates an AGENTS.md file, which is the "brain" OpenCode uses to understand your specific codebase.
